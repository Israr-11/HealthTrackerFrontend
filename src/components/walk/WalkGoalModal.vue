    ...
    <div class="modal-dialog animate__animated animate__fadeInDown">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title">Walk Goal</h5>
          <button type="button" class="btn-close" @click="$emit('close')" aria-label="Close"></button>
        </div>
        <form @submit.prevent="createGoal">
          <div class="modal-body">
            <div class="mb-3">
              <label for="targetSteps" class="form-label">Target Steps</label>
              <input
                  type="number"
                  id="targetSteps"
                  v-model="targetSteps"
          ...
          <div class="modal-body">
            <div class="mb-3">
              <label for="uphill" class="form-label">Target Walk's Track</label>
              <select
                  id="uphill"
                  v-model="uphill"
                  class="form-control"
                  required
              >
                <option value="" disabled class="text-black-50">Uphill Track</option>
...
<script>
import { createWalkGoal} from "@/services/walk.js";
...
export default {
  ...
  methods: {
    async createGoal() {
      ...
      try {
        const response = await createWalkGoal(newGoal);
