<template>
  <div>
    <div class="d-flex justify-content-between align-items-center mb-1 mt-5">
      ...
      <button class="btn btn-primary" @click="showModal = true">Create Walk Goal</button>
    </div>
    ...
    <table class="table table-striped">
      ...
      <tr>
        ...
        <th>Walk ID</th>
        <th>Target Steps</th>
        <th>Target Track in Uphill</th>
        <th>Entry Time</th>
        <th>Enter Walk Log</th>
      </tr>
      </thead>
      <tbody>
      ...
      <div class="modal-dialog animate__animated animate__fadeInDown">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title">Enter Log for Walk</h5>
            <button
                type="button"
                class="btn-close"
                @click="closeLogModal"
          ...
          <form @submit.prevent="AddLog">
            ...
            <div class="modal-body">
              <div class="mb-3">
                <label for="actualSteps" class="form-label">Actual Steps</label>
                <input
                    type="number"
                    id="actualSteps"
                    v-model="actualSteps"
                    class="form-control"
                    placeholder="e.g., 500, 600, 10000, 11000....."
                    required
                />
              </div>
            </div>
...
export default {
  ...
  methods: {
    //FETCHING THE GOALS FOR WALK FEATURE
    async fetchGoals() {
      const user = JSON.parse(localStorage.getItem("user"));
      const userId = user.id;
      try {
        const scheduleResponse = await getWalkGoalsByUserId(userId);
    ...
    async AddLog() {
      const user = JSON.parse(localStorage.getItem("user"));
      const userId = user.id;
      const walkGoalId = this.selectedGoal.id;
